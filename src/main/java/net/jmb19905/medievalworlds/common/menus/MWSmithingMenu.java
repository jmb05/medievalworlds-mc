...
import net.jmb19905.medievalworlds.common.recipes.SmithingRecipe;
...
public class MWSmithingMenu extends MWInventoryMenu<SmithingTableBlockEntity> {
    ...
    public MWSmithingMenu(int windowId, Inventory inventory, SmithingTableBlockEntity blockEntity) {
    ...
    public MWSmithingMenu(int windowId, Inventory inventory, FriendlyByteBuf byteBuf) {
    ...
    public Predicate<BlockState> isValidBlockState() {
        return state -> state.is(VanillaOverride.SMITHING_TABLE.get());
    }
    ...
    private List<Recipe<?>> getRecipes() {
        ...
        List<SmithingRecipe> listSmithing = this.level.getRecipeManager().getRecipesFor(MWRecipeSerializers.SMITHING_TYPE, customSmithingTypeWrapper, this.level);
    ...
    private Container getSlotsAsContainer() {
        return new SimpleContainer(blockEntity.getInventory().toNonNullList().toArray(ItemStack[]::new));
    }
